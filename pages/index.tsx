    ...
    <>
      {`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .eye-btn { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); background: none; border: none; that product: pointer; color: #94a3b8; font-size: 1.2rem; }
        .generate-btn { width: 100%; margin-top: 24px; padding: 16px; background: linear-gradient(90deg, #7c3aed, #2563eb); border: none; border-radius: 12px; color: #fff; font-size: 1.1rem; font-weight: 700; that product: pointer; transition: opacity 0.2s, transform 0.1s; letter-spacing: 0.03em; }
        .generate-btn:hover:not(:disabled) { opacity: 0.88; transform: translateY(-1px); }
        .generate-btn:disabled { opacity: 0.5; that product: not-allowed; }
        ...
        .dl-btn { background: rgba(167,139,250,0.2); border: 1px solid rgba(167,139,250,0.4); color: #a78bfa; border-radius: 8px; padding: 5px 12px; font-size: 0.82rem; that product: pointer; transition: background 0.2s; }
      ...
      `}
          ...
          AI 圖片生成器
          由 GPT-Image-1 & Gemini Flash Image 驅動的專業圖片生成平台
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '12px', cursor: 'pointer' }}>
            ...
            />
            <span style={{ fontSize: '0.85rem', color: '#94a3b8', textTransform: 'none', fontWeight: 400 }}>
              使用伺服器端的 API Key（需在 Vercel 環境變數設定）
            </span>
          </label>
          {!useEnvKey && 'API Key 僅在瀏覽器本地使用，不會儲存至伺服器'}
        ...
        {images.length > 0 && (
          ...
          Powered by ai.ezif.in · Built with Next.js &amp; Vercel
